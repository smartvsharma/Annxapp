											...
											if(md5(password)==data.password){ console.log("in old password");
												 ...
												 password = md5(password);
													userObj.updateUser2(userId,{'password':password},function(err,data){ console.log("after update"+JSON.stringify(data));														   if (err){
																  ...
																  if (data.n >0) {
																		 var response = {"status":'success',"messageId":"200","message":"Password updated successfully"};
																		 res.status(200).json(response);        
																  }else{
																		 var response = {"status":'faliure',"messageId":"401","message":"Something went wrong, password not updated."};
																		 res.status(200).json(response);        
																  }
